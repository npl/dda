...
import dda.viewer.BigGrid;
import dda.viewer.BigTile;
import dda.viewer.RePaintAble;
 ...
 * 
 */
public class JMapViewer extends JPanel implements TileLoaderListener, RePaintAble {
	...
	protected boolean densityMapVisible;

	protected TileController tileController;
	private BigTile tileDensityManager;
	...
	public JMapViewer(TileCache tileCache, int downloadThreadCount) {
		super();
		tileDensityManager = new BigTile(this);
			...
			for (int i = 0; i < 4; i++) {
				...
				for (int j = 0; j < x; j++) {
					if (x_min <= posx && posx <= x_max && y_min <= posy && posy <= y_max) {
						...
						if (densityMapVisible && tileDensityManager != null) { 
							tileDensityManager.loadMissingTilesAndPaint(g, tilex, tiley, posx, posy, zoom);
							painted = true;
						}

						if (tileGridVisible) { 
							BigGrid.drawGrid(g, posx, posy, tilesize, zoom);
							painted = true;
						}
					}
					Point p = move[iMove];
	...
	public boolean isDensityMapVisible() {
		return densityMapVisible;
	}

	public void setDensityMapVisible(boolean densityMapVisible) {
		this.densityMapVisible = densityMapVisible;
		repaint();
	}
	...
	public void repaint()
	{
		...
		{
			tileDensityManager.checkAndClearCache(this.getWidth(), this.getHeight(), zoom);
		}
